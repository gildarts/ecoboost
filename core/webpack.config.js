...
module.exports = {
    entry: {
        'ecoboost': './src/main.ts',
        // 'service': './demo/service.ts',
        // 'ecoboost.config': './demo/ecoboost.config.ts',
    }, // 程式進入點。
    output: { //輸出位置。
    ...
    module: {
        rules: [{ // 設定 Typescript 轉譯器(https://github.com/TypeStrong/ts-loader)
            test: /\.ts$/,
            use: 'ts-loader',
            include: [
    ...
    plugins: [
        ...
        new CopyWebpackPlugin([{ // 直接將指定的資源複製到輸出目錄。
                from: 'index.ts',
                from: 'public.d.ts',
            },{
                from: "package.json",
                transform: (content, path) => {
                    ...
                    pkg.main = './ecoboost.js' // 正式環境應該用這個檔案。
